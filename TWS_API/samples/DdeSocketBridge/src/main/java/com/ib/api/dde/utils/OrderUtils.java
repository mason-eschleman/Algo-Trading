   ...
   public static byte[] openOrderDataListToByteArray(List<OrderData> openOrderDataList, List<OrderData> allOpenOrderDataList) {
    ...
    private static ArrayList<String> createCompletedOrderTableItem(OrderData completedOrderData) {
        ...
        if (completedOrderData.order() != null) {
            ...
            item.add(Utils.toString(completedOrderData.order().hedgeMaxSize()));
    ...
    private static ArrayList<String> createOpenOrderTableItem(OpenOrderData openOrderData, int index) {
        ...
        if (openOrderData.order() != null) {
            ...
            item.add(Utils.toString(openOrderData.order().hedgeMaxSize()));
            ...
            item.add(Utils.toString(openOrderData.order().postOnly()));
            item.add(Utils.toString(openOrderData.order().allowPreOpen()));
            item.add(Utils.toString(openOrderData.order().ignoreOpenAuction()));
            item.add(Utils.toString(openOrderData.order().deactivate()));
            item.add(Utils.toString(openOrderData.order().seekPriceImprovement()));
            item.add(Utils.toString(openOrderData.order().routeMarketableToBbo()));
        } else {
            for (int j = 0; j < 116; j++) {
                item.add(Utils.toString(""));
            }
